...
(defun read-triplet (stream)
  (+ (read stream) (* 256 (read stream)) (* 256 256 (read stream))))
...
(defun load-map (file)
  (with-open-file (*in* file :direction :input)
	...
	(with-output-to-string (str)
	  (dotimes (x w)
	    (if (eq #xffffff (read-triplet *in*))
		(format str "-")
		(format str "*")))
	  (push (get-output-stream-string str) *map*)))
      (setf *map* (list-to-vector (nreverse *map*))))))
